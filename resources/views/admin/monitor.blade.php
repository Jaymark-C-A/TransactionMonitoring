    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper bg-transparent">
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    <div class="current-ticket smoky-shadow m-0 p-0" style="background: #20b6bb">
                        <div class="card-header mb-5 text-left" style="background: #1eaaaf; color: white;">
                            <h2>CURRENTLY SERVING!</h2>
                        </div>
                        <div class="ticket-number">---</div>
                        <div class="ticket-purpose"></div>
                        <div class="actions">
                            <button class="btn btn-danger no-show" style="border-radius: 0 !important; font-size: 30px !important;">Cancel</button>
                            <button class="btn btn-warning skip" style="border-radius: 0 !important; font-size: 30px !important;">Skip</button>
                            <button class="btn btn-success next" style="border-radius: 0 !important; font-size: 30px !important;">Next</button>
                        </div>
                    </div>
                </div>
            </div>
...
//         var ticketId = $('.current-ticket').attr('data-ticket-id');
//         var assignedDate = $('#assigned-date').val();

//         // Check if the assigned date is provided
//         if (!assignedDate) {
//             alert("Please select an assigned date.");
//             return;
//         }
...
//             data: {
//                 ticket_id: ticketId,
//                 assigned_date: assignedDate // Only the date without time or AM/PM
//             },
//             headers: {
//                 'X-CSRF-TOKEN': csrfToken
//             },
//             success: function(response) {
//                 console.log('Pending info saved:', response);
//                 $('#pendingModal').modal('hide');  // Hide the modal after saving
//                 $('.modal-backdrop').remove();  
//                 // Optionally, refresh the data on the page or update the UI here
//             },
//             error: function(xhr, status, error) {
//                 console.error('Error saving pending info:', error);
...
//             },
//             complete: function() {
//                 // Optional: Remove any loading spinners or overlays if present
//                 // Example: $('.loading-overlay').hide();
//             }
//         });
//     });
//  });
...
{{-- <script>
    function fetchPendingAdminVisitors() {
        fetch('/api/visitors/pending-Admin-info', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('token') // Use token from local storage or session
            }
        })
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            const tableBody = document.getElementById('pending-visitors-table').getElementsByTagName('tbody')[0];
            ...
            data.visitors.forEach(visitor => {
                ...
                row.innerHTML = `
                    <td>${visitor.name}</td>
                    <td><label class="badge badge-primary mx-2">${visitor.status}</label></td>
                    <td>${visitor.assigned_date}</td>
                    <td>${new Date(visitor.created_at).toLocaleString()}</td>
                    <td>
                        <button class="btn btn-primary" data-toggle="modal" data-target="#actionModal" data-ticket-id="${visitor.id}" onclick="PendingAction('${visitor.id}')">Action</button>
                    </td>
                `;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Fetch pending visitors info
        fetchPendingAdminVisitors();

        // Set up interval to fetch pending count and visitors info every 1 minute
        setInterval(() => {
            fetchPendingAdminVisitors();
        }, 1000); // 1 minute
    });
...
<script>
    ...
    let selectedVisitorId; // Global variable to hold the selected visitor ID

    function PendingAction(visitorId) {
    selectedVisitorId = visitorId; // Set the selected visitor ID
    fetch(`/api/visitors/${visitorId}`)
        .then(response => response.json())
        .then(visitor => {
            if (visitor) {
                console.log(visitor);
                document.querySelector('.ticket-number1 #ticketNumber').textContent = visitor.ticket_number;
                document.getElementById('visitorIdInput').value = visitorId; // Set the hidden input
 ...
 function handleNoShow() {
    Swal.fire({
        title: 'Are you sure?',
        text: "You are about to mark this visitor as canceled.",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            document.getElementById('cancelVisitorIdInput').value = selectedVisitorId; // Set the visitor ID
            document.getElementById('cancelForm').submit(); // Submit the cancel form if confirmed
        }
    });
 }
