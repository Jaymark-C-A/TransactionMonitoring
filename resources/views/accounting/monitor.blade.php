    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper bg-transparent">
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    <div class="current-ticket smoky-shadow m-0 p-0" style="background: #097969;">
                        <div class="card-header mb-5 text-left" style="background: #076e5e; color: white;">
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var filteredData = data.filter(function(item) {
                            return item.status !== 'completed' && item.status !== 'canceled' && item.status !== 'skipped' && item.department;
                        });
                        ...
                        if (filteredData.length === 0 || filteredData.length === 1) {
                            ...
                            html += '<p class="mb-0" style="font-size:20px; text-align:left;"><strong>No Queueing Tickets Available</strong></p>';
                            html += '<p class="mb-0">Please check back later or add new tickets.</p>';
                            html += '</div>';
                        } else {
                        ...
                        //     return item.status !== 'completed' && item.status !== 'canceled' && item.status !== 'waiting' && item.status !== 'serving';
                        // });

                        // // Exclude the first ticket
                        // var filteredDataWithoutFirst = filteredData1.slice(1); 

                        // var html = '<div class="card-container p-2" style="width: 65%;">';

                        // // Check if there are tickets to display
                        // if (filteredDataWithoutFirst.length === 0) {
                        //     html += '<div class="card" style="background-color: #f8d7da;">'; // Light red background
                        //     html += '<p class="mb-0" style="font-size:20px; text-align:left;"><strong>No Queueing Tickets Available</strong></p>';
                        //     html += '<p class="mb-0">Please check back later or add new tickets.</p>';
                        //     html += '</div>';
                        // } else {
                        //     filteredDataWithoutFirst.forEach(function(visitor) {
                        //         html += '<div class="card" style="background-color: #66b5e93d">';
                        //         html += '<p class="mb-0" style="font-size:24px;"><strong>' + (visitor.ticket_number || 'N/A') + '</strong></p>';
                        //         html += '<p class="mb-0"><strong>Purpose:</strong> ' + (visitor.purpose || 'Not specified') + '</p>';
                        //         html += '</div>';
                        //     });
                        // }
